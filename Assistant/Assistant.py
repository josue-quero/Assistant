...
"""
Assistant, is an interface to facilitate
the analissis and download of
mexican high school certificates
...
class DownloadTask(QtCore.QObject):
    ...
    saving_error = QtCore.pyqtSignal(AssistantWarning)
    ...
    def saveFile(self, overwrite=False):
        try:
            ...
            self.saving_error.emit(AssistantWarning.INVALID_NAME)
            return

        if not Path(self._folder).exists():
            self.saving_error.emit(AssistantWarning.FOLDER_NOT_EXISTS)
            return

        if self._output_file.exists() and not overwrite:
            self.saving_error.emit(AssistantWarning.FILE_EXISTS)
            return
...
class AssistantModel:
    ...
    _filename_format = "certificate_data_{batch_number:03}.json"
    _config_filename = "Assistant_config.json"
    ...
    def createDownload(self, batch_number, threaded):
        if self.download_count < self._max_downloads:
            return AssistantWarning.MAX_REACHED
        ...
        if preexistent_task is not None:
            return AssistantWarning.DOWNLOAD_EXISTS
